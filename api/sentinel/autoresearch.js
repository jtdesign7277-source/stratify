...
function applyTweak(currentParams) {
  ...
  return { newParams, tweakedKey: key, oldVal, tweakedVal: newVal };
}
...
async function logExperiment(tag, iteration, tweakedKey, oldVal, tweakedVal, result, score, status, params, previousParams) {
  ...
  const avgR = result.tradeCount > 0 ? result.totalPnl / result.tradeCount : 0;
  ...
  const { error } = await supabase.from('sentinel_experiments').insert({
    ...
    iteration,
    parameters: params,
    previous_parameters: previousParams,
    changed_param: tweakedKey,
    changed_from: String(oldVal),
    changed_to: String(tweakedVal),
    ...
    composite_score: score,
    status,
    description: `tweak ${tweakedKey}: ${oldVal} → ${tweakedVal}`,
    created_at: now,
    completed_at: now,
  });

  if (error) console.error('[autoresearch] logExperiment insert error:', error);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (let iter = 1; iter <= maxIter; iter++) {
      ...
      const { newParams, tweakedKey, oldVal, tweakedVal } = applyTweak(currentParams);
      const result = runBacktest(bars, newParams);
      allPnls.push(result.totalPnl);
      iterData.push({ iter, newParams, tweakedKey, oldVal, tweakedVal, result, previousParams });
    }
    ...
    for (const { iter, newParams, tweakedKey, oldVal, tweakedVal, result, previousParams } of iterData) {
      ...
      await logExperiment(tag, iter, tweakedKey, oldVal, tweakedVal, result, score, status, newParams, previousParams);
