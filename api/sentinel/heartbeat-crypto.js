...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (existingSession) {
      ...
      await supabase.from('sentinel_sessions').insert({ session_date: today, trades_fired: newSignals, trades_closed: tradesClosed, wins: sessionWins, losses: sessionLosses, gross_pnl: parseFloat(sessionGrossPnl.toFixed(2)), session_started_at: new Date().toISOString() });
    }
