...
export default function AdvancedChartsPage({ activeTicker = 'NVDA' }) {
  ...
  return (
    <motion.div {...PAGE_TRANSITION} className="flex h-full w-full bg-[#060a12] text-white">
      ...
      >
        <div className="border-b border-white/10 bg-[#060d18] px-3 py-2">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-blue-300/85">
              Order Entry
            </span>
            <button
              type="button"
              ...
              className="inline-flex h-7 w-7 items-center justify-center text-white/45 transition hover:text-white/80"
            >
            ...
            <AlpacaOrderTicket
              ...
              extraFields={
                <div className="space-y-2">
                  {orderType === 'limit' && (
                    <div>
                      <label className="mb-1 block text-[12px] font-semibold text-slate-200">
                        Limit Price
                      </label>
                      <input
                        type="number"
                  ...
                  {orderSide === 'sell' && sizeMode === 'shares' && (
                    <div className="text-[11px] text-white/45">{availableSharesDisplay} shares available</div>
                  )}
                  {orderError && <div className="text-[11px] text-red-300">{orderError}</div>}
                </div>
              }
            />
          </div>
