...
import { Search, Zap, TrendingUp, Activity, BarChart3, Flame, Sparkles } from 'lucide-react';
...
const StrategyTemplatesGallery = ({ onSelectTemplate = () => {} }) => {
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#0d0d12] text-white">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-white/5 py-2 pl-9 pr-3 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
            />
          </div>
        </div>
            ...
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${
                activeCategory === category
                  ...
                  : 'border-white/10 bg-white/5 text-gray-400 hover:text-white hover:border-emerald-400/40'
              }`}
            >
              {category}
            </button>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          {featuredTemplate && (
            <div className="md:col-span-2 lg:col-span-3">
              <div className="relative overflow-hidden rounded-2xl border border-emerald-400/30 bg-gradient-to-br from-[#141424] via-[#0f1017] to-[#0b0b12] p-6 shadow-[0_0_35px_rgba(16,185,129,0.15)]">
                ...
                <div className="relative z-10">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/20 border border-emerald-400/30 px-3 py-1 text-xs font-semibold uppercase tracking-[0.15em] text-emerald-200">
                  ...
                  <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="space-y-3">
                      <h3 className="text-2xl font-semibold text-white">{featuredTemplate.name}</h3>
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <button
                        ...
                        className="mt-2 inline-flex items-center justify-center rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-[#06140c] shadow-[0_0_20px_rgba(16,185,129,0.35)] transition hover:-translate-y-0.5 hover:bg-emerald-400"
                      >
                        Use Template
                      </button>
                    </div>
          ...
          {standardTemplates.map((template) => (
            <div
              key={template.id}
              className="flex h-full flex-col rounded-2xl border border-white/10 bg-white/5 p-5 transition hover:border-emerald-400/30"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-base font-semibold text-white">{template.name}</h3>
                  <span className="mt-2 inline-flex items-center rounded-full border border-white/10 px-2.5 py-1 text-xs text-gray-400">
                    {template.category}
                  </span>
                </div>
              ...
              <p className="mt-3 text-sm text-gray-400 flex-1">{template.description}</p>
              ...
              <button
                ...
                className="mt-5 inline-flex items-center justify-center rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-medium text-white transition hover:border-emerald-400/40 hover:bg-emerald-500/10"
              >
                Use Template
              </button>
            </div>
