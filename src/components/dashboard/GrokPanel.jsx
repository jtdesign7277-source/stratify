...
const GrokPanel = () => {
  ...
  const handleBuild = async () => {
    ...
    const tfLabel = selectedTimeframe ? TIMEFRAMES.find(t => t.id === selectedTimeframe)?.label : '';
...
${selectedTimeframe ? `Backtest period: ${tfLabel}` : 'No specific backtest timeframe.'}
Include entry/exit conditions and Python code.`;

    setMessages(prev => [...prev, { role: 'user', content: `Build ${strategyType.name} for ${tickerList}` }]);
    ...
    try {
      ...
      const fullContent = data.response || "Couldn't generate. Try again.";
  ...
  return (
    <div className="w-64 h-full bg-[#060d18] border-l border-gray-800 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 p-2.5 flex flex-col gap-2.5 overflow-hidden">
        ...
        <div className="flex-shrink-0">
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded text-[11px] font-medium transition-all ${
                  selectedTickers.includes(s)
                    ...
                    : 'bg-[#0d1829] text-gray-400 border border-gray-700 hover:border-emerald-500/30'
                }`}
              >
                ${s}
              </button>
          ...
          <div className="relative">
            <div className="flex items-center gap-1 bg-[#0d1829] border border-gray-700 rounded px-2 py-1">
              ...
              <input
                ...
                className="flex-1 bg-transparent text-white placeholder-gray-600 text-[11px] outline-none"
              />
              {tickerSearch && <button onClick={() => setTickerSearch('')}><X className="w-3 h-3 text-gray-500" /></button>}
            </div>
            {searchResults.length > 0 && (
              <div className="absolute left-0 right-0 top-full mt-0.5 bg-[#0d1829] border border-gray-700 rounded z-50">
                {searchResults.map(t => (
                  <div key={t.symbol} onClick={() => addTicker(t.symbol)} className="flex items-center justify-between px-2 py-1 hover:bg-emerald-500/20 cursor-pointer text-[11px]">
                    <span><span className="text-white">${t.symbol}</span> <span className="text-gray-500">{t.name}</span></span>
            ...
            <div className="flex flex-wrap gap-1 mt-1.5">
              {selectedTickers.filter(s => !['QQQ', 'SPY', 'TSLA', 'NVDA', 'BTC'].includes(s)).map(s => (
                <span key={s} className="flex items-center gap-0.5 px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 border border-emerald-500/50 rounded text-[10px]">
                  ${s}<button onClick={() => removeTicker(s)}><X className="w-2.5 h-2.5" /></button>
                </span>
              ))}
            </div>
          )}
            ...
            {STRATEGY_TYPES.map(s => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[9px] block mt-0.5 ${selectedStrategy === s.id ? 'text-emerald-400' : 'text-gray-500'}`}>{s.name}</span>
                </button>
              );
            })}
          </div>
              ...
              <button
                ...
                className={`flex-1 py-1 rounded text-[11px] font-medium transition-all ${
                  selectedTimeframe === tf.id
                    ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50'
                    : 'bg-[#0d1829] text-gray-500 border border-gray-700 hover:border-gray-600'
                }`}
              >
                {tf.label}
              </button>
        ...
        <div className="flex-shrink-0">
          ...
          <input
            ...
            className="w-full px-2 py-1 bg-[#0d1829] border border-gray-700 rounded text-white placeholder-gray-600 text-[11px] focus:outline-none focus:border-emerald-500"
          />
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
              ...
              <div className="p-1.5 space-y-1.5">
                {messages.map((m, i) => (
                  <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[95%] rounded px-2 py-1 ${
                      m.role === 'user' ? 'bg-emerald-600 text-white' : 'bg-[#0d1829] text-gray-300 border border-gray-800'
                    }`}>
                      {m.role === 'assistant' && (
          ...
          <div className="flex gap-1 flex-shrink-0">
            <textarea
              ...
              rows={2}
              className="flex-1 px-2 py-1.5 bg-[#0d1829] border border-gray-700 rounded text-white placeholder-gray-600 text-[11px] resize-none focus:outline-none focus:border-emerald-500"
            />
            <button
              onClick={handleChatSend}
              disabled={!chatInput.trim() || isChatLoading}
              className={`px-2 rounded transition-all ${chatInput.trim() && !isChatLoading ? 'bg-emerald-600 hover:bg-emerald-500 text-white' : 'bg-gray-800 text-gray-600'}`}
            >
      ...
      {/* Build Button */}
      <div className="p-2.5 border-t border-gray-800 flex-shrink-0">
        <button
          onClick={handleBuild}
          disabled={selectedTickers.length === 0 || !selectedStrategy || isGenerating}
          className={`w-full py-2 rounded-lg font-medium text-sm flex items-center justify-center gap-1.5 transition-all ${
            selectedTickers.length > 0 && selectedStrategy && !isGenerating
              ? 'bg-emerald-600 hover:bg-emerald-500 text-white'
              : 'bg-gray-800 text-gray-500 cursor-not-allowed'
          }`}
        >
          {isGenerating ? <><Loader2 className="w-4 h-4 animate-spin" />Building...</> : 'Build'}
        </button>
      </div>
    </div>
  );
