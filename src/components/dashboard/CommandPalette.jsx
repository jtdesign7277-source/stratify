...
export default function CommandPalette({ 
  ...
  const allCommands = useMemo(() => {
    const commands = [
      ...
      { id: 'nav-builder', name: 'Strategy Workspace', description: 'Open Terminal strategy folders', category: 'navigation', icon: Layout, shortcut: 'Cmd+3', action: () => onNavigate?.('terminal') },
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (cmdKey && e.key >= '1' && e.key <= '5') {
        e.preventDefault();
        const tabs = ['dashboard', 'terminal', 'terminal', 'arbitrage', 'settings'];
