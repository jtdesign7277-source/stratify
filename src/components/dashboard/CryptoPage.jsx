...
function OrderEntry({
  ...
  const handleSubmit = () => {
    if (!hasValidOrderSize || trading || !symbolForTrade) return;
    setConfirmModal(true);
  };

  const executeOrder = async () => {
    if (!symbolForTrade || trading) return;
    ...
    try {
      ...
      const result = side === 'sell'
        ? await sell(symbolForTrade, resolvedQuantity)
        : await buy(symbolForTrade, resolvedQuantity);
