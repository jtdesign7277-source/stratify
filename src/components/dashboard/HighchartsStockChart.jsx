...
async function fetchHistoricalData(symbol, interval = '1day', outputsize = 500, apiKey = '') {
  ...
  if (data.values && Array.isArray(data.values)) {
    data.values.forEach((bar) => {
      ...
      // FIX: use _o and _c keys to match volume color mapping below
...
export default function HighchartsStockChart({
  ...
  const buildOpts = useCallback((ohlc, vol) => {
    const up = theme.up;
    const dn = theme.down;
    // FIX: keys are _c and _o (set correctly in fetchHistoricalData above)
    const cVol = vol.map((v) => ({
      x: v.x, y: v.y,
      color: v._c >= v._o ? up + '44' : dn + '44',
      borderColor: v._c >= v._o ? up + '77' : dn + '77',
    }));
    ...
    return {
      chart: {
        ...
        animation: false,
        spacing: [0, 0, 0, 0],
        // ─── FIX: disable ALL built-in Highcharts pan/zoom so our custom
        //         handlers have exclusive control — no more conflicts ───
        panning: { enabled: false },
        zooming: {
          type: undefined,
          mouseWheel: { enabled: false }, // custom wheel handler takes over
          pinchType: 'x',                 // keep pinch-to-zoom on touch
        },
      },
      ...
      navigator: {
        ...
        handles: { backgroundColor: '#1f2937', borderColor: '#4a5568' },
      },
      scrollbar: {
      ...
      xAxis: {
        gridLineWidth: 0, lineColor: '#1a2332', tickColor: '#1a2332', crosshair: false,
        labels: { style: { color: '#4a5568', fontSize: '10px' } },
        minPadding: 0, maxPadding: 0.06, overscroll: rightOffsetMs,
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.error(err); setSeriesSeed({ ohlc: [], volume: [] }); setError('Failed to load data'); setLoading(false);
    }
  }, [symbol, interval]);
  ...
  const applyRangePreset = useCallback((rangeValue, options = {}) => {
    ...
    const now = dataMax;
    const rangeMsMap = { '1D': 86_400_000, '5D': 5 * 86_400_000, '1M': 30 * 86_400_000, '3M': 90 * 86_400_000, '6M': 180 * 86_400_000, '1Y': 365 * 86_400_000 };
    if (rangeValue === 'ALL') { xAxis.setExtremes(dataMin, end, true, false, { trigger: 'range-preset' }); if (persist) setActiveRange('ALL'); return; }
  ...
  const fitChartViewport = useCallback((rangeValue = activeRange) => {
    ...
    if (yAxis) yAxis.setExtremes(null, null, false, false, { trigger: 'fit-y' });
    applyRangePreset(rangeValue, { persist: false });
    chart.redraw(false);
  }, [activeRange, applyRangePreset]);

  const chartOptions = useMemo(() => buildOpts(seriesSeed.ohlc, seriesSeed.volume), [buildOpts, seriesSeed.ohlc, seriesSeed.volume]);
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        let socketUrl = '';
        if (TD_API_KEY) { socketUrl = `${TD_WS_URL}?apikey=${encodeURIComponent(TD_API_KEY)}`; }
        else {
          const cfgResponse = await fetch(`/api/lse/ws-config?symbols=${encodeURIComponent(symbol)}`, { cache: 'no-store' });
          const cfgPayload = await cfgResponse.json().catch(() => ({}));
          if (!cfgResponse.ok || !cfgPayload?.websocketUrl) throw new Error(cfgPayload?.error || `ws-config failed (${cfgResponse.status})`);
    ...
    return () => {
      cancelled = true;
      try { wsRef.current?.send(JSON.stringify({ action: 'unsubscribe', params: { symbols: symbol } })); } catch {}
  ...
  useEffect(() => {
    ...
    const onMouseDown = (event) => {
      ...
      const insidePlot = px >= chart.plotLeft && px <= chart.plotLeft + chart.plotWidth && py >= chart.plotTop && py <= chart.plotTop + chart.plotHeight;
      if (!insidePlot) return;
      dragStateRef.current = {
        startX: event.clientX, startY: event.clientY,
        xMin: xAxis.min, xMax: xAxis.max, yMin: yAxis.min, yMax: yAxis.max,
    ...
    const onMouseMove = (event) => {
      ...
      if (nextXMin < leftLimit) { const shift = leftLimit - nextXMin; nextXMin += shift; nextXMax += shift; }
      ...
      if (nextYMax > yMaxLimit) { const shift = nextYMax - yMaxLimit; nextYMin -= shift; nextYMax -= shift; }
    ...
    const onWheel = (event) => {
      ...
      const mouseRatio = (px - chart.plotLeft) / Math.max(chart.plotWidth, 1);
      const newRange = xRange * factor;
      const xCenter = xAxis.min + xRange * mouseRatio;
      let newMin = xCenter - newRange * mouseRatio;
      let newMax = xCenter + newRange * (1 - mouseRatio);

      // Clamp to data bounds
      const dataMin = Number.isFinite(xAxis.dataMin) ? xAxis.dataMin : newMin;
      const dataMax = (Number.isFinite(xAxis.dataMax) ? xAxis.dataMax : newMax) + rightOffsetMs;
      const minRange = getIntervalMs(interval) * 2;
      if (newMax - newMin < minRange) { // don't over-zoom in
        newMin = xCenter - minRange * mouseRatio;
        newMax = xCenter + minRange * (1 - mouseRatio);
      }
  ...
  }, [rightOffsetMs, interval]); // FIX: added interval to deps so minRange stays current
  ...
  const handleZoom = useCallback((direction) => {
    ...
    if (nextMin < dataMin) { nextMin = dataMin; nextMax = Math.min(paddedDataMax, dataMin + nextRange); }
    if (nextMax > paddedDataMax) { nextMax = paddedDataMax; nextMin = Math.max(dataMin, paddedDataMax - nextRange); }
  ...
  useEffect(() => {
    const fn = () => { setIsFs(!!document.fullscreenElement); setTimeout(() => { chartRef.current?.chart?.reflow(); fitChartViewport(activeRange); }, 120); };
  ...
  useEffect(() => {
    ...
    const reflow = () => { chartRef.current?.chart?.reflow(); fitChartViewport(activeRange); };
    ...
    return () => { clearTimeout(timer); observer?.disconnect(); window.removeEventListener('resize', reflow); };
  ...
  const handleDraw = (tool) => {
    const chart = chartRef.current?.chart;
    if (!chart) return;
    if (tool.type === 'clearAll') { if (chart.annotations) while (chart.annotations.length) chart.removeAnnotation(chart.annotations[0]); }
    else if (chart.navigationBindings) { chart.navigationBindings.selectedButton = tool.type; }
    setShowDraw(false);
  };
  ...
  const P = 'px-2.5 py-0.5 text-[16px] font-medium rounded cursor-pointer transition-all duration-100 select-none whitespace-nowrap';
  ...
  return (
    <div ref={containerRef} style={{ display: 'flex', flexDirection: 'column', width: '100%', height: h, maxHeight: maxChartHeight, backgroundColor: '#000', position: 'relative', overflow: 'hidden', borderRadius: isFs ? '0' : '6px', border: isFs ? 'none' : '1px solid #111827' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '3px 8px', backgroundColor: '#000', borderBottom: '1px solid #111827', flexShrink: 0, whiteSpace: 'nowrap', overflowX: 'auto' }}>
        <div style={{ position: 'relative' }} data-dd>
          <button onClick={() => setShowSym(!showSym)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '4px 9px', backgroundColor: '#111827', border: '1px solid #1f2937', borderRadius: '5px', cursor: 'pointer', color: '#e5e7eb', fontSize: '17px', fontWeight: '700' }}>
            <span style={{ color: '#4a5568', fontSize: '14px' }}>$</span>{symbol}<span style={{ color: '#4a5568', fontSize: '11px', marginLeft: '2px' }}>▼</span>
          </button>
          {showSym && (
            <div style={{ ...DD, minWidth: '200px' }}>
              ...
              {filteredWl.map((s) => (
                <button key={s} onClick={() => pickSymbol(s)} style={{ ...DI(s === symbol), fontWeight: s === symbol ? '600' : '400' }}
                  onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.04)')}
                  onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = s === symbol ? 'rgba(59,130,246,0.1)' : 'transparent')}>
        ...
        <span style={{ color: '#e5e7eb', fontSize: '20px', fontWeight: '700', fontFamily: "'SF Mono', monospace" }}>{fmtPrice(hover.c)}</span>
        {hover.chg != null && <span style={{ color: hover.chg >= 0 ? '#22c55e' : '#ef4444', fontSize: '16px', fontWeight: '600', fontFamily: "'SF Mono', monospace" }}>{hover.chg >= 0 ? '+' : ''}{hover.chg?.toFixed(2)} ({hover.pct?.toFixed(2)}%)</span>}
        {isLive && <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', color: '#22c55e', fontWeight: '600' }}><span style={{ width: '7px', height: '7px', borderRadius: '50%', backgroundColor: '#22c55e', animation: 'pulse 2s infinite' }} />LIVE</div>}

        <div style={{ flex: 1 }} />

        <span style={{ color: '#6b7280', fontSize: '14px', fontWeight: '600' }}>Size: Fill</span>

        <button onClick={toggleFs} className={`${P} ${PF}`} title="Fullscreen">{isFs ? '⊡' : '⛶'}</button>
        <button onClick={() => handleZoom('in')} className={`${P} border border-emerald-500/50 text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10`} title="Zoom in" style={{ fontSize: '32px', lineHeight: 1, fontWeight: 700 }}>＋</button>
        <button onClick={() => handleZoom('out')} className={`${P} border border-red-500/50 text-red-400 hover:text-red-300 hover:bg-red-500/10`} title="Zoom out" style={{ fontSize: '32px', lineHeight: 1, fontWeight: 700 }}>－</button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '2px', padding: '2px 8px', backgroundColor: '#000', borderBottom: '1px solid #0a0a0a', flexShrink: 0, flexWrap: 'nowrap', overflowX: 'auto', whiteSpace: 'nowrap' }}>
        {TOP_INDICATOR_CHIPS.map((chipId) => {
          const chip = INDICATOR_OPTIONS.find((opt) => opt.id === chipId);
          if (!chip) return null;
          const active = activeInd.includes(chipId);
          return <button key={chipId} onClick={() => toggleInd(chipId)} className={`${P} ${active ? PN : PF}`}>{chip.label}</button>;
        })}
          ...
          {showInd && (
            <div style={DD}>
              {INDICATOR_OPTIONS.map((ind) => (
                <button key={ind.id} onClick={() => toggleInd(ind.id)} style={DI(activeInd.includes(ind.id))}
                  onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.04)')}
                  onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = activeInd.includes(ind.id) ? 'rgba(59,130,246,0.1)' : 'transparent')}>
        ...
        <div style={{ position: 'relative' }} data-dd>
          ...
          {showDraw && (
            <div style={DD}>
              {DRAWING_TOOLS.map((t) => (
                <button key={t.label} onClick={() => handleDraw(t)} style={{ ...DI(false), color: t.type === 'clearAll' ? '#ef4444' : '#8892a0' }}
                  onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.04)')}
        ...
        <div style={{ position: 'relative' }} data-dd>
          ...
          {showColor && (
            <div style={DD}>
              {CANDLE_THEMES.map((t) => (
                <button key={t.label} onClick={() => { setTheme(t); setShowColor(false); }} style={DI(theme.label === t.label)}
                  onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.04)')}
                  onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = theme.label === t.label ? 'rgba(59,130,246,0.1)' : 'transparent')}>
                  <span style={{ display: 'flex', gap: '2px' }}>
                    <span style={{ width: '8px', height: '14px', backgroundColor: t.up, borderRadius: '1px' }} />
                    <span style={{ width: '8px', height: '14px', backgroundColor: t.down, borderRadius: '1px' }} />
                  </span>{t.label}{theme.label === t.label && <span style={{ marginLeft: 'auto', fontSize: '9px' }}>✓</span>}
                </button>
              ))}
            </div>
          )}
      ...
      {/* ── OHLCV hover bar ── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '3px 10px', backgroundColor: '#000', flexShrink: 0, fontFamily: "'SF Mono', monospace", fontSize: '14px', whiteSpace: 'nowrap', overflowX: 'auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 10px', backgroundColor: '#000', borderTop: '1px solid #0f1722', flexShrink: 0, fontFamily: "'SF Mono', monospace", whiteSpace: 'nowrap', overflowX: 'auto' }}>
        <span style={{ color: '#4a5568', fontSize: '14px' }}>Range:</span>
          ...
          {showRangeMore && (
            <div style={{ ...DD, minWidth: '90px' }}>
              {RANGE_PRESETS_MORE.map((item) => (
                <button key={item.value} onClick={() => { applyRangePreset(item.value); setShowRangeMore(false); }} style={DI(activeRange === item.value)}>{item.label}</button>
              ))}
            </div>
          )}
        </div>
        <div style={{ width: '1px', height: '14px', backgroundColor: '#1a2332', margin: '0 4px' }} />
        <span style={{ color: '#4a5568', fontSize: '14px' }}>Interval:</span>
