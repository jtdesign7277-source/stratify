      ...
      {!collapsed && (
        <>
          ...
          <div className="w-full border-t border-white/5 py-3">
            <div className="px-3 pt-4 pb-1 text-xs uppercase tracking-widest transition-all duration-200" style={{ color: T.muted }}>EXPLORE</div>
              ...
              {EXPLORE_TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full py-2 px-3 rounded-lg inline-flex items-center gap-3 text-sm font-normal transition-colors ${isActive ? 'text-[#e6edf3] bg-white/5' : 'text-[#7d8590] hover:text-[#e6edf3] hover:bg-white/3'}`}
                  >
                    <Icon className="h-4 w-4" strokeWidth={1.5} />
                    <span className="text-sm transition-all duration-200">{tab.label}</span>
                  </button>
                );
              })}
            </div>
...
// ─── History View ─────────────────────────────────────────
const HistoryView = ({ history, loading, onClear }) => {
  const relTime = (iso) => {
    if (!iso) return '';
...
// ─── Discover View ────────────────────────────────────────
const DiscoverView = ({ data, loading }) => {
  if (loading || !data) {
    return (
...
// ─── Finance View ─────────────────────────────────────────
const FinanceView = ({ data, loading }) => {
