...
export default function ChartWrapper({ title, subtitle, options, loading = false, error = null, empty = false }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0a1628] p-4">
      ...
      {loading ? (
        <div className="flex h-[280px] items-center justify-center text-xs text-[#9ca3af]">Loading chart...</div>
      ) : null}

      {!loading && error ? (
        <div className="flex h-[280px] items-center justify-center text-xs text-red-300">{error}</div>
      ) : null}

      {!loading && !error && empty ? (
        <div className="flex h-[280px] items-center justify-center text-xs text-[#9ca3af]">No data available</div>
      ) : null}
