import { useMemo } from 'react';
import ChartWrapper from './ChartWrapper';

const toSeries = (arr = []) => arr.map((v) => (Number.isFinite(Number(v)) ? Number(v) : 0));

export function getFreeCashFlowConfig(labels, operatingCF, investingCF) {
  const op = toSeries(operatingCF);
  const inv = toSeries(investingCF);
  const freeCF = op.map((v, i) => v + (inv[i] || 0));

  return {
    chart: { type: 'areaspline', height: 260 },
    title: { text: null },
    xAxis: { categories: labels },
    yAxis: {
      title: { text: null },
      labels: {
        formatter() {
          return '$' + (this.value / 1e3).toFixed(0) + 'B';
        },
      },
      plotLines: [{ value: 0, color: 'rgba(255,255,255,0.08)', width: 1 }],
    },
    tooltip: { valuePrefix: '$', valueSuffix: 'M' },
    legend: { enabled: true, align: 'left', verticalAlign: 'top' },
    series: [
      {
        name: 'Operating CF',
        data: op,
        color: '#3b82f6',
        fillColor: {
          linearGradient: { x1: 0, y1: 0, x2: 0, y2: 1 },
          stops: [
            [0, 'rgba(59,130,246,0.2)'],
            [1, 'rgba(59,130,246,0)'],
          ],
        },
        lineWidth: 2,
        marker: { radius: 4 },
      },
      {
        name: 'Free Cash Flow',
        data: freeCF,
        color: '#10b981',
        fillColor: {
          linearGradient: { x1: 0, y1: 0, x2: 0, y2: 1 },
          stops: [
            [0, 'rgba(16,185,129,0.2)'],
            [1, 'rgba(16,185,129,0)'],
          ],
        },
        lineWidth: 2,
        marker: { radius: 4 },
      },
    ],
  };
}

export default function FreeCashFlowChart({ labels = [], operatingCF = [], investingCF = [] }) {
  const options = useMemo(
    () => getFreeCashFlowConfig(labels, operatingCF, investingCF),
    [investingCF, labels, operatingCF]
  );

  return (
    <ChartWrapper
      title="Free Cash Flow Trend"
      subtitle="Operating cash flow vs free cash flow"
      options={options}
      height={260}
    />
  );
}
